...
double wheelCircumference = 1.2767; //meters
double wheelDistance = 0.4699; //meters
...
double string_to_double(const std::string& s1) {
  std::istringstream i(s1);
  double x;
...
void odomCallback(const std_msgs::String::ConstPtr & msg) {
  ...
  enL1 = -round(string_to_double(in[0]));
  enL2 = -round(string_to_double(in[1]));
  enR1 = -round(string_to_double(in[2]));
  enR2 = -round(string_to_double(in[3]));
